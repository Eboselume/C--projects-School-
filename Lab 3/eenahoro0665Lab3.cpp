/* Lume Enahoro
	Professor Hogan 
    Lab 3
	This program is about using different sorting techniques for a set of numbers in ascending order
	4 October 2019
 */
	...
	do
	{
		...
		for (int count = 0; count < (size - 1); count++)
		{
			if (*ptr[count] > *ptr[count + 1])
			{
				...
				//calls the function displayPtrArrayDeref
				displayPtrArrayDeref(ptr, size);
			}
		}
	} while (swap);
...
void SelectionSort(const int *ptr[], int size)
{
	...
	*/
	...
	for (startScan = 0; startScan < (size - 1); startScan++)
	{
		...
		//changed the relational sign in order to get ascendong order
		...
		//Call displayPtrArrayDeref function
		displayPtrArrayDeref(ptr, size);
	}
}
...
void InsertionSort(const int *ptr[], int size)
{
	...
	*/
	...
	for (int i = 1; i < size; i++)
	{
		...
		//changed the relational sign on order to get ascending order
	...
	//Call displayPtrArrayDeref function again
		displayPtrArrayDeref(ptr, size);
	}
}